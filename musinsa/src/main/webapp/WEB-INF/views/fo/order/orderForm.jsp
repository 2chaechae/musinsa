...
<script>
	...
	DD_RUM.onReady(function() {
		DD_RUM.init({
			clientToken: 'pub220673779508d5d9a78e0a885bc72e69',
			applicationId: 'e627d077-f0c0-45ae-8586-c9d6fec75291',
			site: 'datadoghq.com',
    ...
    "orderInfo" : {
        "list" : [
            ...
            { "id": "3058265-0",
                "price": "162000",
				"quantity": "1",
                "goods_nm": "벌룬핏 미니멀 데님 점퍼 블루 IEJU3E212B2",
				"category": "Clothing > 점퍼/재킷 > 데님/트러커 재킷",
				"brand": "ilcorso",
                "option": "00M",
				"season": "2023 S/S",
                "sex": "2",
				"goods_type": getGoodsTypeBySignatureSaleYn("P", "N"),
				"normal_price": "169000",
				"sale_rate": "4",
				"specialty_code" : ""
            }
...
function getGoodsTypeBySignatureSaleYn(goodsType, signatureSaleYn) {
	if (signatureSaleYn === 'Y') {
		return goodsType + "B";
	} else {
		return goodsType;
	}
...
<!--order.css-->
<link type="text/css" rel="stylesheet" href="//static.msscdn.net/skin/musinsa/css/order.min.css?202303281956" /><script type="text/javascript" src="//static.msscdn.net/skin/musinsa/js/miya_validator.js?20160119"></script>
<script type="text/javascript" src="//static.msscdn.net/skin/musinsa/js/order.js?202304070920"></script>
<script type="text/javascript" src="//static.msscdn.net/skin/musinsa/js/mpay.js?202301131036"></script>
<script type="text/javascript" src="//static.msscdn.net/skin/musinsa/js/md5.js?20160119"></script>
<script type="text/javascript" src="//static.msscdn.net/skin/musinsa/js/refund-account.js?202404070011"></script>
<script type="text/javascript" src="//static.msscdn.net/skin/musinsa/js/popup_window.js?202101201140"></script>

<script type="text/javascript">
window.name = "order_form_";
// var kcppay = true;		// KCP AX 결제창
var user_id = '';
var total_prepoint_amt = '810';
var total_point_amt = '810';
var plcc_open_yn = 'N';
var digital_product_order_yn = 'N';
const mobile_certify_yn = 'N';
const guestYn = getCookie("guest");

//<![CDATA[
$(document).ready(function() {
	Order.checkIncludesDigitalProduct();
	Order.initPayAmt();
    if (guestYn !== 'Y') {
        Order.initMemberRecentPayment();
    }
	if (plcc_open_yn == 'Y') {
		PLCC.setPLCCData(); // 즉시할인 되는 카드 자동 셋팅 및 즉시할인 금액 적용
	}
	initDiscountAmt();
	if (Order.isMsspaySupported()) {
		Msspay.showCashReceiptByPaymentType();
		Msspay.setImmediateDiscount(); // 즉시할인 금액 적용된 총 결제금액 표시
	}

	if (typeof(initRecommendId) == 'function') {
		initRecommendId(user_id);
	}
...
$(window).on('pageshow', function (event) {
    if (event.originalEvent.persisted || (window.performance && window.performance.navigation.type == 2)) {
        var checked_order_discount = getCookie("checked_order_discount");

		if (guestYn != 'Y' && checked_order_discount && checked_order_discount == "true") {
			if (document.getElementById("pointUse")) {
				document.getElementById("pointUse").checked = true;
			}
			document.getElementById("prepointUse").checked = true;
		}
    }
});

$(document).on('click', '.order__tooltip--bottom__close', function (e) {
	const uiTooltipObject = $(e.target).parents('.order__tooltip--bottom');
	const tooltipInner = uiTooltipObject.prev('.order__tooltip').attr('data-tooltip');
	const tooltipActive = tooltipInner + `--active`;

	if (uiTooltipObject.hasClass(tooltipActive)) {
		if ($(e.target).closest('.order__tooltip').length === 0) {
			uiTooltipObject.removeClass(tooltipActive);
		}
	}
});

function isCheckedOrderDiscount() {
	var checked_order_discount = getCookie("checked_order_discount");
	var discount_applied_yn = document.getElementById("discount_applied_yn");

	if (checked_order_discount && checked_order_discount == "true" && discount_applied_yn && discount_applied_yn.value== 'y') {
		return true;
	}
	return false;
}

function initDiscountAmt() {
	var use_yn = 'N';

	if (guestYn != 'Y') {
		use_yn = isCheckedOrderDiscount() ? 'Y' : 'N';
		setPointUse(use_yn);
		setPrepointUse(use_yn);
		setCouponData(use_yn);

		// 상품별 할인가가 판매가를 초과하는지 체크
		Order.isDiscountAmtExceedsPriceByProduct();
	}
}

// 적립금 선할인
function setPrepointUse(use_yn) {
	if (document.getElementById("prepointUse")) {
		if (use_yn == 'Y') {
			document.getElementById("prepointUse").checked = true;
		} else {
...
// 보유 적립금
function setPointUse(use_yn) {
	if (document.getElementById("pointUse")) {
		if (use_yn == 'Y') {
			document.getElementById("pointUse").checked = true;
		} else {
			document.getElementById("pointUse").checked = false;
		}
...
// 장바구니에서 적용된 쿠폰 적용
function setCouponData(use_yn) {
	if (use_yn == 'Y') {
		var coupon_data = getBrowserData('coupon');

		if (coupon_data && coupon_data != '') {
			var coupon_json = JSON.parse(coupon_data);

			var a_coupon_use_data = [];
			var sum_dc_amt = 0;

			for (var i = 0; i < coupon_json.length; i++) {
				var goods_coupon_info = coupon_json[i].goods_coupon_info;
				var exist_goods_check = document.getElementById("goods_" + coupon_json[i].goods_no + "_" + coupon_json[i].goods_sub + "_" + coupon_json[i].opt);

				if (exist_goods_check) {
					if (goods_coupon_info && goods_coupon_info != "") {
						var a_goods_coupon_info = goods_coupon_info.split("|");
						// 쿠폰 데이터 적용
						var coupon_amt_new = ApplyCouponData(goods_coupon_info);
						a_goods_coupon_info[5] = coupon_amt_new;
						var goods_coupon_info_new = a_goods_coupon_info.join("|");
						a_coupon_use_data.push(goods_coupon_info_new);
						// 총 할인액 계산
						sum_dc_amt += parseInt(a_goods_coupon_info[5], 10);
					}
				}
			}
			var coupon_use_data = '';
			if (a_coupon_use_data) {
				coupon_use_data = a_coupon_use_data.join("\t");
			}

			CheckPayCoupon(sum_dc_amt, coupon_use_data);
			CalculateAmt();
		}
	} else {
		setBrowserData('coupon', '');
	}
}

function ApplyCouponData(goods_coupon_info) {
	var a_goods_coupon_info = goods_coupon_info.split("|");
	var goods_no = a_goods_coupon_info[0];
	var goods_sub = a_goods_coupon_info[1];
	var cart_num = a_goods_coupon_info[2];
	var coupon_no = a_goods_coupon_info[3];
	var coupon_idx = a_goods_coupon_info[4];
	var goods_dc_amt = a_goods_coupon_info[5];
	...
	if(coupon_idx == "" ) {
		$.ajax({
			type: "POST",
			async: false,
			url: '/app/product/svc_coupon_down_and_apply',
			dataType: 'json',
			...
			success: function(json) {
				...
				if(result == 1) {
					...
					alert('쿠폰 동시 이용 회원이 많아\n쿠폰이 적용되지 않았습니다.\n다시 시도해 주세요.');
				} else if(result == 600) {
					alert('비정상 요청으로 쿠폰 발급이 실패했습니다.');
				} else if(result == 700) {	// 쿠폰다운로드 재 요청에 의한 2초 제한
					alert('2초 후 다시 시도하여 주십시오.');
	...
	// 쿠폰 할인액 상품가 초과여부 확인
	var goods_dc_amt_2 = Order.isDiscountAmtExceedsPriceByProduct(cart_num, goods_dc_amt);
	var goods_dc_amt_new = ! goods_dc_amt_2 ? goods_dc_amt : goods_dc_amt_2;

	$.ajax({
		type: "get",
		url: "/app/order/coupon_available_cmd",
		async: false,
		data: "is_ajax=1&layer_cnt=1&cart_num=" + cart_num + "&goods_coupon_info_0=" + goods_no + "|" + goods_sub + "|" + cart_num + "|" + coupon_no + "|" + coupon_idx + "|" + goods_dc_amt_new,
		success: function (msg) {
			if (msg != 1) {
				alert("쿠폰 적용 중 문제가 발생했습니다. 다시 시도하여 주십시오.");
			}
		}
	});

	return goods_dc_amt_new;
}

// 첫결제즉시할인 클릭
function checkPLCCFirstPayDiscount() {
	var clicked_yn = 'N';
...
function CheckPayCoupon(totalCouponDCAmt, couponUseData) {
	var ff = document.f1;
	ff.kcoupon.value = totalCouponDCAmt;

	var totalPayAmt = Order.getPayAmt(); // 총 결제금액
	if (totalPayAmt < 1000) {
	...
	couponData = couponUseData.split("\t");
	var couponAppliedCartNo = [];
	for(var i=0; i < couponData.length; i++) {
		var data = couponData[i].split("|");
		var cartNo = data[2];
		var couponAmt = data[5];

		couponAppliedCartNo.push(cartNo);
		var couponAmt2 = Order.isDiscountAmtExceedsPriceByProduct(cartNo, couponAmt);
		couponAmt = ! couponAmt2 ? couponAmt : couponAmt2;
		if (couponAmt === couponAmt2) {
			$('[name="limit_coupon_amt_yn_'+ cartNo +'"]').val('Y');
		} else {
			$('[name="limit_coupon_amt_yn_'+ cartNo +'"]').val('N');
		}

		$("input[name='coupon_amt_" + cartNo + "'][type='hidden']").val(couponAmt);
	}

	// 쿠폰 적용이 안된 상품의 경우 쿠폰 금액제한 여부 N 처리
	var cartInfo = $("input[name='cart_info'][type='hidden']");
	for(var i = 0; i < cartInfo.length; i++) {
		var info = $(cartInfo[i]).val().split('|');
		var cartAllNo = info[9];
		if (couponAppliedCartNo.indexOf(cartAllNo) < 0) {
			$('[name="limit_coupon_amt_yn_'+ cartAllNo +'"]').val('N');
			$("input[name='coupon_amt_" + cartAllNo + "'][type='hidden']").val(0);
		}
	}
...
/*선적립금 텍스트 출력*/
function CheckPrepoint(prepoint, point) {
	if(document.getElementById("prepointUse").checked) {
		if(prepoint==point) {
			document.getElementById("prepoint_chk").innerHTML = "선할인";
		} else {
			document.getElementById("prepoint_chk").innerHTML = Comma(prepoint) + " 원 할인 /" + Comma(point-prepoint) + " 원 적립";
		}
		document.getElementById("total_point_amt").innerHTML = Comma(point-prepoint);
	} else {
		if(prepoint==point) {
			document.getElementById("prepoint_chk").innerHTML = "적립";
		} else {
			document.getElementById("prepoint_chk").innerHTML = 0 + " 원 할인 /" + Comma(point) + " 원 적립";
		}
		document.getElementById("total_point_amt").innerHTML = Comma(point);
	}
}

function ChoiceQuickDeliveryAddress(no) {
	$.ajax({
		type: "POST",
		url: "/app/mypage/get_delivery/" + no,
		success: function(msg) {
			eval("var json = " + msg);
			if (json.zipcode && Order.validZipCode(json.zipcode)) {
				setDelivery(json);
			}
		}
	});
...
function getQuickDeliveryAddress(no) {
	$.ajax({
		type: "POST",
		url: "/app/delivery/get_quickdelivery/" + no,
		dataType: 'json',
		success: function(json) {
			if (json.isSuccess) {
				var data = json.data;
				var delivery_html = "";
				$.each(data.new_delivery, function (index, list) {
					delivery_html +=
							'<li>' +
								'<input type="radio" onclick="ChoiceQuickDeliveryAddress(' + list.no + ')" class="n-radio" id="delivery_choice_' + index + '" name="delivery_choice"' + ((list.no == no) ? 'checked' : '') + '>' +
									'<label for="delivery_choice_'+ index +'">' + list.title_quick + '</label>' +
							'</li>';
				});
				$("#quickDeliveryList").html(delivery_html);
				checkedIfFirstDeliveryAddress(data.new_delivery);

				if (Object.keys(data.delivery_info).length > 0) {
...
function checkedIfFirstDeliveryAddress(newDeliveryObj) {
	if (Object.keys(newDeliveryObj).length === 1) {
		$('[name=delivery_choice]').prop('checked', true);
		ChoiceQuickDeliveryAddress(newDeliveryObj[0].no);
		showDeliveryData();
	}
}

function showDeliveryData() {
	$('.delivery__item__none').remove();
	$('.delivery__item__info').show();
}

function setDelivery(obj) {
	let ff = document.f1;

	ff.rcvr_nm.value = obj.name;
	ff.rtitle.value = obj.title;
	ff.rzip_cd1.value = obj.zipcode1;
	ff.rzip_addr1.value = obj.addr1;
	ff.rzip_addr2.value = obj.addr2;
	ff.rmobile1.value = obj.mobile1;
	ff.rmobile2.value = obj.mobile2;
	ff.rmobile3.value = obj.mobile3;
	ff.rphone1.value = (obj.phone !== "" && obj.phone !== "--") ? obj.phone1 : '';
	ff.rphone2.value = (obj.phone !== "" && obj.phone !== "--") ? obj.phone2 : '';
	ff.rphone3.value = (obj.phone !== "" && obj.phone !== "--") ? obj.phone3 : '';
	ff.dlv_msg.value = obj.dlv_msg;
	ff.rselectedNo.value = obj.no;

	const deliveryTitle = obj.title;
	const deliveryName = obj.name;
	const deliveryMobile = obj.mobile1 + "-" + obj.mobile2 + "-" + obj.mobile3;
	const deliveryPhone = (obj.phone !== "" && obj.phone !== "--") ? obj.phone1 + "-" + obj.phone2 + "-" + obj.phone3 : '';
	const deliveryAddress = "(" + obj.zipcode1 + ") " + obj.addr1 + " " + obj.addr2;

	$("#delivery-title").html(deliveryTitle);
	$("#delivery-name").html(deliveryName);
	$("#delivery-mobile").html(deliveryMobile);
	$("#delivery-phone").html(deliveryPhone);
	$("#delivery-addr").html(deliveryAddress);

	// 배송메시지 셋팅
	const $dlv_msg_option = $('#dlv_selectbox option[value="' + obj.dlv_msg + '"]');
	if (obj.dlv_msg == null) {
		$('#dlv_selectbox option:first').prop('selected', true);
		showEtc($('#dlv_selectbox').val());
	} else if ($dlv_msg_option.length > 0) {
		$dlv_msg_option.prop('selected', true);
		showEtc($('#dlv_selectbox').val());
	} else {
		$('#dlv_selectbox option[value="etc"]').prop('selected', true);
		showEtc($('#dlv_selectbox').val());
		$('#etc_textarea').val(obj.dlv_msg);
	}
}

function pay_info_area_show(payKind) {
	$("#right_pay_info_area").show();
    $('#right_pay_info_area > .box_payment_event').hide();

	$("#right_pay_info_" + payKind).show();
	$("[name=pay_info]").show();
}

function pay_info_view(payKind) {
    // 오른쪽 영역
    $("#right_pay_info_area").show();
    $('#right_pay_info_area > .box_payment_event').hide();
    $("#right_pay_info_" + payKind).show();
    ...
    // 하단 영역
    $("[name=pay_info]").show();
    $('.box-payKind-info').hide();
    $('#box-payKind-info-' + payKind).show();

    $('#box-pay-cash-receipt').hide();
    if (payKind == 'VTRANSFER') {
        $('#box-pay-cash-receipt').show();
    }
}
...
function addGiftList(obj) {
	var gift_no = obj.id;
	var is_chk = $("#"+gift_no).is(":checked");
	if(is_chk) {
		var gift_nm = $("#gift_nm_"+gift_no).val();
		var list_tag = "<li id=\"gift_list_" + gift_no + "\">"+ gift_nm +"<a href=\"#\" onClick=\"delGiftList('"+ gift_no +"'); return false;\"><span class=\"delete-btn-red\">X</span></a></li>";
		$("#gift_list").append(list_tag);
	} else {
		delGiftList(gift_no);
	}
}

function delGiftList(gift_no) {
	$("#gift_list_" + gift_no).remove();
	$("#"+ gift_no).prop("checked", false);
}

function SetRecipientPhoneNumber(obj) {
	var ff = document.f1;
	...
	ff.rphone1.disabled = isChecked;
	ff.rphone2.disabled = isChecked;
	ff.rphone3.disabled = isChecked;
}

function SetBuyerPhoneNumber(obj) {
	var ff = document.f1;
	var isChecked = $(obj).is(":checked");
	if(isChecked) {
		ff.ophone1.value = '';
	...
	ff.ophone1.disabled = isChecked;
	ff.ophone2.disabled = isChecked;
	ff.ophone3.disabled = isChecked;
}

function checkMobileCretify(obj, mobile_certify_yn) {
	if(mobile_certify_yn != "Y") {
		obj.checked = false;
		alert("가상계좌 결제는 회원가입 및 휴대폰인증 후 사용이 가능합니다.");
		return false;
	}
    return true;
}
...
function chkMaxUserPoint(point) {
...
const selectPayKind = function(payKind) {
    if (payKind === 'VTRANSFER') {
        if ($('input[name=virtual_block_yn]').val() === 'Y') {
            alert('가상 계좌 주문의 반복적인 취소로 가상 계좌 주문이 제한됩니다');
            return false;
        } else if (checkMobileCretify(document.getElementById('btn-paykind-VTRANSFER'), mobile_certify_yn) === false) {
            return false;
        }
    }

	if (!Order.validGeneralPayKind(payKind)) {
		const payKindName = $('#btn-paykind-' + payKind).data('pay-kind-name');
		alert(payKindName + ' 점검중입니다.\n다른 결제 수단을 이용해주세요.');
		Order.setPayment('', '', '', '');
		return false;
	}
    ...
    setPayKind(payKind);
    CalculateAmt();
    pay_info_area_show(payKind);
    pay_info_view(payKind);
    showClaim();
}
