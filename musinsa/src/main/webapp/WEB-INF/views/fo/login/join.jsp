        ...
        <div id="joinContainer" class="login-join-membership">
            ...
            <div class="login-join-membership__form">
                ...
                <div class="login-input" :class="{'login-input--layer': emailDomainAutoCompleteList.length > 0 }">
                    ...
                    <p v-html="emailValidMessage" v-show="emailValidMessage"
                       :class="[emailValid ? 'login-input__helper-text--blue' : 'login-input__validation']" v-cloak></p>
                ...
                <div class="login-join-membership__form__agreement" >
                    <div class="login-checkbox login-join-membership__all-checkbox">
                        ...
                        <label for="loginJoinMembershipAllCheckbox" class="login-checkbox__label login-join-membership__all-checkbox__label">약관 전체 동의하기 (선택 동의 포함)</label>
                    </div>
                    ...
                    <div class="login-checkbox--version login-join-membership__checkbox">
                        ...
                        <button @click="popUpForTerms('/member/join/agreement/privacy-usage-v2')" class="login-checkbox--version__link">자세히</button>
                    </div>
                    <div class="login-checkbox--version login-join-membership__checkbox">
                    ...
                    <div class="login-checkbox--version login-join-membership__checkbox">
                        <input v-model="memberPrivacyAgree" type="checkbox" id="memberPrivacyAgreeCheckbox" name="memberPrivacyAgreeCheckbox" class="blind">
                        <label for="memberPrivacyAgreeCheckbox" class="login-checkbox--version__label login-join-membership__checkbox__label">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" class="login-checkbox--version__svg">
                                <title></title>
                                <path d="M2.6665 8L6.39952 11.7333L13.8665 4.26666" stroke="#D1D1D1" stroke-width="1.5" stroke-linecap="square" class="svg-color" />
                            </svg> [선택] 개인정보 수집 및 이용 동의
                        </label>
                        <button @click="popUpForTerms('/member/join/agreement/member-privacy-agree')" class="login-checkbox--version__link">자세히</button>
                    </div>
                </div>
            </div>
            <div class="login-button login-button--static" v-cloak>
                <button v-if="kakaoJoin" type="submit" class="login-button__item login-button__item--blue" :disabled="!readyToJoin || joinButtonClicked" @click="join()">
                    카카오 간편 가입 완료</button>
                <button v-else-if="appleJoin" type="submit" class="login-button__item login-button__item--blue" :disabled="!readyToJoin || joinButtonClicked" @click="join()">
                    본인인증하고 가입하기</button>
            ...
            <ul class="login-join-membership__guide-list" v-cloak>
                ...
                <li v-if="!hyundaiCardJoin && !kakaoJoin && !app" class="login-join-membership__guide-list__normal">본인인증이 어려운 경우 <a :href="linkForNonMember" class="login-join-membership__guide-list__link">비회원으로 구매</a>할 수 있습니다.</li>
            </ul>
        </div>
    </section><!-- //content -->
...
<script type="text/javascript">
    ...
    (function () {
        ...
        function getAmplitudeType() {
            const joinType = 'SELF_CERTIFY'
        ...
        AppInterface.sendScreenEvent(
            'screen_member_join', {
                ...
                type: getAmplitudeType()
            }
        );

        function joinButtonSendAmplitudeEvent () {
            const eventType = getAmplitudeType();

            let buttonName = '';
            if (eventType === 'musinsa') {
                buttonName = 'join';
            }

            if (eventType === 'apple') {
                buttonName = 'apple_join';
            }

            if (eventType === 'kakao') {
                buttonName = 'kakao_join';
            }

            AppInterface.sendEvent(
                'click_button', {
                    from: 'screen_member_join',
                    to: 'screen_join_complete',
                    section_name: 'member',
                    type: eventType,
                    button_name: buttonName,
                    url: 'https://www.musinsa.com/member/join/complete'
                }
            );
        }
        ...
        const vue = new Vue({
            ...
            data: {
                ...
                memberPrivacyAgree: false,
                memberPrivacyAgreeEnabled: false,
            ...
            mounted () {
                ...
                this.appleJoin = this.joinType === "APPLE"
                this.passwordCheckPass = this.joinType == "APPLE"
                this.memberPrivacyAgreeEnabled = "true"
            ...
            methods: {
                ...
                turnAllTerms: function (value) {
                    if (value) {
                        ...
                        vue.memberPrivacyAgree = true
                    } else {
                        vue.privacyAgree = false
                        vue.useTermsAgree = false
                        vue.over14Agree = false
                        vue.marketingReceiveAgree = false
                        vue.memberPrivacyAgree = false;
                    }
                },
                checkAllTermsTrue: function () {
                    vue.agreeAll = (vue.privacyAgree && vue.useTermsAgree && vue.over14Agree && vue.marketingReceiveAgree && vue.memberPrivacyAgree);
                },
                ...
                join: function () {
                    ...
                    axios.post(
                        ...
                        this.toFormData({
                                ...
                                "checkPhoneNumber": "0",
                                "memberPrivacyAgree": vue.memberPrivacyAgree,
                                "memberPrivacyAgreeEnabled": vue.memberPrivacyAgreeEnabled,
                        ...
                        .then((response) => {
                            ...
                            if (!data.success) {
                                ...
                                if (this.storeCode.trim() !== "" || this.storeCode !== null) {
                ...
                memberIdValidResultSendAmplitude: function (code) {
                    if('MEMBER-002-1004' === code) {
                        AppInterface.sendEvent(
                            'check_fail_message', {
                                from: 'screen_member_join',
                                section_name: 'id',
                ...
                emailValidResultSendAmplitude: function (code) {
                    if('MEMBER-002-1201' === code) {
                        AppInterface.sendEvent(
                            'check_fail_message', {
                                from: 'screen_member_join',
                                section_name: 'email',
                ...
                recommendMemberIdValidResultSendAmplitude: function () {
                    AppInterface.sendEvent(
                        'check_fail_message', {
                            from: 'screen_member_join',
                            section_name: 'recommend',
                            type: getAmplitudeType()
                        }
                    );
                },
            ...
            watch: {
                memberId: _.debounce(function (value) {
                    ...
                    axios.post(mypageUrl + "/api/member/v2/join/memberId/exist", {memberId: value})
                        .then(function (response) {
                            ...
                            vue.memberIdValidResultSendAmplitude(data.code);
                        })
                        .catch(function (data) {
                ...
                email: _.debounce(function (value) {
                    ...
                    axios.post(mypageUrl + "/api/member/v2/join/email/exist", {email: value.toLowerCase()})
                        .then(function (response) {
                            ...
                            vue.emailValidMessage = data.usermessage
                            vue.emailValidResultSendAmplitude(data.code)
                        })
                        .catch(function (data) {
                    ...
                    axios.post(mypageUrl + "/api/member/v2/join/verify/recommend-member-id", {recommendMemberId: value})
                        .then(function (response) {
                            ...
                            if (data.success === true) {
                                vue.recommendMemberIdValidMessage = "추천인 아이디입니다. 가입 후 추천인과 신규회원 모두 적립금 5,000원을 드립니다"
                            } else {
                                vue.recommendMemberIdValidMessage = "추천인 아이디를 확인해주세요.<br>추천인은 친구 초대 이벤트에 참여한 이력이 있어야 합니다.<br>(참여 경로: 마이페이지 > 포인트 > 친구 초대 미션)"
                                vue.recommendMemberIdValidResultSendAmplitude()
                            }
                        })
                        .catch(function (data) {
                ...
                memberPrivacyAgree: function (value) { vue.checkAllTermsTrue() },
            },
            computed: {
