...
function getShowcaseList(){
	...
	var page_index = document.getElementById('showcase_list').getAttribute('page_index');
	var $container = $('#showcase_list');
	var total = document.f1.total.value;

	$.ajax({
		type: "POST",
		url: "/app/svc/get_showcase_list",
		data: 'page=' + page_index + '&state=' + state + '&brand=' + brand + '&page=' + page_index + '&display_cnt=' + display_cnt ,

		success: function(msg){
			if(msg){

				page_index++;
				document.getElementById('showcase_list').setAttribute('page_index',page_index);

				$container.append( msg );

				var list_cnt = total - $("#showcase_list li").length;
